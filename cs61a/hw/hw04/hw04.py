...
def prune_leaves(t, vals):
    ...
    if is_leaf(t):
        ...
        q = []
        for x in branches(t):
            if prune_leaves(x, vals):
                q.append(prune_leaves(x, vals))
#        y = [prune_leaves(x, vals) for x in branches(t) if prune_leaves(x, vals)]
        return tree(label(t), q)
...
def balanced(m):
    ...
    if is_weight(m):
        return  True
    balanced1 = length(m[1]) * total_weight(end(left(m))) == length(m[2]) * total_weight(end(right(m)))
    balanced2 = balanced(m[1][2]) == balanced(m[2][2])
    return balanced1 and balanced2
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(total_weight(m),[totals_tree(m[1][2]),totals_tree(m[2][2])])
